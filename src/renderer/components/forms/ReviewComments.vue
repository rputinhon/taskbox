<template>
  <v-menu rounded="lg" right nudge-left="20" ref="reviewcomments" offset-x :close-on-content-click="false">
    <template v-slot:activator="{ on: onMenu }">
      <v-badge dot overlap color="warning" offset-y="10" :value="badge && comments.length" class="mr-2">
        <v-btn fab fixed icon x-small right rounded class="mr-0" v-on="{ ...onMenu }">
    ...
    <v-card min-width="300px" max-width="300px" max-height="350px" min-height="350px" height="350px" color="grey lighten-2" style="overflow: hidden">
      <v-progress-linear v-if="loading" absolute top indeterminate />
      <v-layout fill-height column align-center justify-end style="height: 305px">
...
export default {
  ...
  props: { reviewItem: Object, badge: Boolean },
  name: 'ReviewComments',
  data() {
    return {
      loading: false,
  ...
  computed: {
    ...
    comments() {
      return this.reviewItem.comments;
    },
  },
  methods: {
    isMe(id) {
    ...
    close() {
      this.$refs.reviewcomments.isActive = false;
    },
  },
};
</script>
