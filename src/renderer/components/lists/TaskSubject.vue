      ...
      <template v-slot:activator="{ on: onHover }">
        <v-tooltip bottom transition="none">
          <template v-slot:activator="{ on: onTooltip }">
            <v-btn class="windowbar-button ma-0" icon :absolute="options.includes('absolute')" :x-large="options.includes('x-large')" :small="!options.includes('small')" :x-small="options.includes('x-small')" v-on="{ ...onHover, ...onTooltip }" elevation="0" @click.stop="getTask(taskId);">
