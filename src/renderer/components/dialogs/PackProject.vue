<template>
  <v-dialog v-if="showPackWindow" v-model="showPackWindow" persistent width="300">
    <v-card rounded="lg" class="py-10 px-5">
      <svg width="50" height="50" version="1.1" viewBox="0 0 50 55" xmlns="http://www.w3.org/2000/svg">
        <path
      ...
      <v-card-title class="text-center" style="display: block"> Pack TaskBox </v-card-title>
      <v-card-subtitle class="py-0 my-0" style="line-height: 15px; min-height: 30px">
        <span v-if="!packing && !packComplete">Creates a structured zip file for the current taskbox.</span>
        <span v-else v-text="!packComplete ? 'packing ...' : 'Pack Complete!'">packing {{ packingTaskBox.title }}</span>
      </v-card-subtitle>
      <v-card-text>
        <v-list-item-content style="height: 150px">
          <v-list-item-title class="text-h5 text-center pt-4" style="display: block"> {{ packingTaskBox.title }}_{{ version }} </v-list-item-title>
          <v-row align="center" justify="center" class="my-3" style="height: 80px">
            <v-text-field  :disabled="packing || packComplete" label="version" style="max-width: 65px" type="number" v-model="version" dense hide-details="true" outlined :min="0" />
          </v-row>
        </v-list-item-content>
        <v-card-subtitle style="height: 40px">
          <small v-show="fileExist && !packComplete" class="red--text"> Do you want to overwrite it? </small>
        </v-card-subtitle>
      </v-card-text>
      <v-card-actions>
        <v-row class="py-1" align="center" justify="center" style="width: 100%">
          <v-btn rounded :disabled="packing" small color="secondary" class="mr-2" @click="close()">close</v-btn>
          <v-btn rounded :disabled="packing" small @click="packComplete ? openFolder() : packProject()" :color="packComplete ? 'success' : fileExist ? 'error' : 'primary'" v-text="!packComplete ? (fileExist ? 'overwrite' : 'pack') : 'open location'"></v-btn>
        </v-row>
      </v-card-actions>
      <v-progress-linear bottom absolute v-if="packing" :color="packComplete ? 'success' : packing ? 'primary' : 'grey'" :indeterminate="packing" />
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  async mounted() {
    await this.$store.dispatch('taskbox/GET_PACK_LIST');
  },
  watch: {
    packingTaskBox(value) {
      if (!value) {
  ...
  computed: {
    ...mapState({
      packingTaskBox: (state) => state.taskbox.packingTaskBox,
      packList: (state) => state.taskbox.packList,
    }),
    fileExist() {
      this.refreshKey;
      return this.packList.find((pack) => pack.name.split('.')[0] == `${this.packingTaskBox.title}_${this.version}`);
    },
  },
  methods: {
    async openFolder() {
    ...
    async task(data) {
      return new Promise((res) => {
        ipcRenderer
          .invoke('app:pack-Project', { root: data.root, children: data.children, paths: data.paths, overwrite: this.overwrite, version: this.version })
          .then((result) => {
            console.log(result);
            this.refreshKey++;
          })
    ...
    async packProject() {
      this.packing = true;
      this.$store.dispatch('taskbox/PACK_TASKBOX', this.packingTaskBox.id).then(async (res) => {
        await this.task(res);
        this.packing = false;
    ...
    async close() {
      this.$store.commit('taskbox/CLOSE_PACKING');
    },
  },
};
</script>
