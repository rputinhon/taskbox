<template>
  <v-app-bar app :flat="analysing !== false" width="auto" style="z-index: 2" height="40px" :dark="analysing !== false && mainTab == 0" :color="analysing == false ? 'white' : 'dark'" class="windowbar px-1" @dblclick="restoreSize()">
    ...
    <v-toolbar height="40" flat color="transparent" class="px-0 text-center windowbar">
      ...
      <v-list-item v-if="analysing == false && mainTab == 0 && dataReady" class="mx-0 mx-1 px-0 windowbar" style="min-height: 40px; height: 40px">
        <task-box-title :options="['info']" v-show="analysing == false" />
        <v-spacer></v-spacer>
        <task-box-menu v-if="analysing == false" :taskbox="currentTaskBox" :options="[analysing !== false && 'white', 'save', 'icon', 'close', 'template', 'reset', 'pack']" />
    ...
    <v-tooltip v-if="mainTab == 0 && dataReady" bottom transition="none">
      <template v-slot:activator="{ on, attrs }">
        <v-btn :disabled="paused || canceled" class="mr-3 windowbar-button" :color="analysing !== false ? 'error' : 'rgba(0,0,0,0.3)'" v-on="on" elevation="0" height="20" width="20" v-bind="attrs" x-small fab @click="toggleAnalisis()" @mouseover.prevent>
