    ...
    <v-system-bar class="mx-auto mt-1" light height="30">
      ...
      <v-menu rounded="0" offset-y top>
        ...
        <v-card class="mx-auto" height="200">
          <v-color-picker v-model="options.fontcolor" hide-canvas hide-inputs hide-sliders hide-mode-switch show-swatches swatches-max-height="200" />
        </v-card>
      </v-menu>
...
export default {
  ...
  computed: {
    ...mapState({
      post: (state) => _.cloneDeep(state.post.post),
    ...
    cap() {
      return this.post.subject.slice(0, 1);
    },
    text() {
      Object.assign(this.post.data, { options: this.options });
      return !this.options.capitalEffect ? this.post.subject : this.post.subject.slice(1, this.post.subject.length);
    },
  },
  methods: {
    changeCap() {
